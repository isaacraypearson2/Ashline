...
- Weapon upgrade (tiers 0→5): `200 + CurrentTier * 250` credits.

## Prestige

`PrestigeReset()` requires rank 50.

Keeps: owned cosmetics, skins, crate history, credits.  
Resets: rank, XP, upgrade tiers, non-starter attachments.  
Grants and equips: `CAMO_PRESTIGE`, `SKIN_GOLD` (all owned guns), `CHARM_SPINE`.
...
| `AshListMeta` | Dump ids to the log |
