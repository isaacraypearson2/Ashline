...
def main() -> int:
    ...
    if missing:
        errors.append(f"Missing DeviceProfiles: {', '.join(missing)}")

    if ("Windows", "Windows") in [(n, b) for n, b in profiles]:
        errors.append("Windows DeviceProfile must not set BaseProfileName=Windows")
    ...
    for cmd in ("AshUnlockAll", "AshGrantCredits", "AshSetRank", "AshUnlockMeta"):
    ...
    soft = read("Source/Ashline/Core/AshlineSoftLoad.h")
    if "TryLoad" not in soft:
        errors.append("AshlineSoftLoad.h missing TryLoad helpers")

    types = read("Source/Ashline/AshlineTypes.h")
    for preset in ("PC_High", "PC_Perf", "SteamDeck"):
        if preset not in types:
            errors.append(f"EAshlineGraphicsPreset missing {preset}")
