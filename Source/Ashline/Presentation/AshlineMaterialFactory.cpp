...
TArray<FString> UAshlineMaterialFactory::MasterGraphCandidates(EAshlineMasterMaterial Master)
{
	TArray<FString> Paths;
	switch (Master)
	{
	case EAshlineMasterMaterial::Environment:
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlineMaster.M_AshlineMaster"));
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlineConcrete.M_AshlineConcrete"));
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/MI_AshlineConcrete.MI_AshlineConcrete"));
		break;
	case EAshlineMasterMaterial::Weapon:
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlineWeapon.M_AshlineWeapon"));
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlineMetal.M_AshlineMetal"));
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/MI_AshlineMetal.MI_AshlineMetal"));
		break;
	case EAshlineMasterMaterial::Character:
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlineCharacter.M_AshlineCharacter"));
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlinePlastic.M_AshlinePlastic"));
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/MI_AshlinePlastic.MI_AshlinePlastic"));
		break;
	case EAshlineMasterMaterial::Decal:
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlineDecal.M_AshlineDecal"));
		Paths.Add(TEXT("/Game/Ashline/Materials/Decals/M_Decal_Master.M_Decal_Master"));
		Paths.Add(TEXT("/Game/Ashline/Materials/Decals/M_Impact_Bullet.M_Impact_Bullet"));
		break;
	case EAshlineMasterMaterial::Glass:
		Paths.Append(UAshlineContentManifest::MasterMaterialCandidates(EAshlineSurface::Glass));
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlineGlass.M_AshlineGlass"));
		break;
	case EAshlineMasterMaterial::Skin:
		Paths.Append(UAshlineContentManifest::MasterMaterialCandidates(EAshlineSurface::Skin));
		Paths.Add(TEXT("/Game/Ashline/Materials/PBR/M_AshlineSkin.M_AshlineSkin"));
		break;
	default:
		break;
	}
...
{
	const TArray<FString> Candidates = UAshlineContentManifest::MasterMaterialCandidates(Surface);
...
{
	TArray<FString> Paths = MasterGraphCandidates(Master);
	switch (Master)
	{
	case EAshlineMasterMaterial::Decal:
...
{
	if (UMaterialInterface* Authored = AshlineMat::LoadFirst<UMaterialInterface>(
		UAshlineContentManifest::MasterMaterialCandidates(Surface)))
	{
		return Authored;
	}
...
{
	if (UTexture* Loaded = AshlineLoad::Soft(Authored))
	{
		return Loaded;
	}
...
FAshlineTextureSet UAshlineMaterialFactory::DefaultsForSurface(EAshlineSurface Surface, const FLinearColor& Tint)
{
	...
	case EAshlineSurface::Asphalt:
		Set.RoughnessValue = 0.82f;
		Set.MetallicValue = 0.05f;
		Set.Tiling = 2.4f;
		break;
	case EAshlineSurface::Dirt:
		Set.RoughnessValue = 0.8f;
		Set.MetallicValue = 0.02f;
		Set.Tiling = 2.2f;
		break;
...
void UAshlineMaterialFactory::StampTextureSet(UMaterialInstanceDynamic* MID, const FAshlineTextureSet& Textures)
{
	...
	UTexture* Roughness = AshlineLoad::Soft(Textures.Roughness);
	UTexture* Metallic = AshlineLoad::Soft(Textures.Metallic);
	UTexture* AO = AshlineLoad::Soft(Textures.AmbientOcclusion);
	...
	const FAshlineTextureSet& Textures)
{
	...
	if (UMaterialInterface* AuthoredMI = AshlineMat::LoadFirst<UMaterialInterface>(
		UAshlineContentManifest::MasterMaterialCandidates(Surface)))
	{
	...
	if (UMaterialInterface* Resolved = UAshlinePresentationLibrary::GetSurfaceMaterial(Surface))
	{
		if (UMaterialInstanceDynamic* MID = UMaterialInstanceDynamic::Create(Resolved, Outer))
		{
			StampTextureSet(MID, Merged);
			return MID;
		}
...
{
	return TEXT("MI_Ashline{Slug} → M_Ashline{Slug} → M_{Slug} → Engine DefaultMaterial / DefaultTexture / DefaultNormal. Missing Quixel never fails PIE/Shipping. Quiet loads via AshlineLoad.");
}
