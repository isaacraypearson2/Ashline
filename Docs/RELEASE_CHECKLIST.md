...
This document is the release gate. Engineering readiness is in this branch. **Marketplace / Fab / authored art is not.**
...
| Steam Deck / Proton profiles | Ready (`Ashline_SteamDeck`, DeviceProfiles, FSR/TSR) | Verify on hardware |
| Input (KBM + gamepad + Deck) | Ready (Enhanced Input runtime IMC) | Verify on Deck |
| Cheat gating in Shipping | Ready (`AshlineShipping::CheatsAllowed`) | No |
| Missing-asset crash hardening | Ready (`AshlineSoftLoad`) | No |
| 12-mission campaign loop | Playable (runtime graybox host) | No for “playable demo”; **yes** for AAA store page |
...
| Empty Fab paths log (Verbose / Warning) | Low | Soft-load returns null; blockout / Engine mesh is used |
...
### Audio
- [ ] Fire / reload / hit / footstep cues on the seven weapons **or** accept silence
