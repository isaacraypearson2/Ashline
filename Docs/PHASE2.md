...
9. UMG frontend / **armory / operator locker** (Canvas HUD + C++ meta remain playable). Bind `MeshOverride` / `MaterialOverride` on cosmetics, skins, equipment.
10. Package a **Win64 Development** build and profile Lumen/RT at 1440p. Also smoke `AshSteamDeck` / `AshPCPerf`.

Overnight catalogs + gunfeel already live: **`Docs/PHASE_OVERNIGHT.md`**.
