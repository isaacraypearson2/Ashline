...
Public SKU is **Win64 Shipping** running under **Proton**. A native SteamOS Linux cook is optional later; it is **not** the first depot.

Named handheld DeviceProfiles (`Ashline_SteamDeck`, Linux/SteamDeck roots) live in **PR #8**. This branch owns runtime `AshDeck` + input + Shipping notes, not a second copy of those INI rows.

## Detection

Runtime (`UAshlineGameUserSettings::IsSteamDeckHardware` / `DetectSteamDeck`):

- Environment `SteamDeck=1` (SteamOS / Proton sets this)
- Command line `-steamdeck`
- Typical 1280×800 handheld resolution

On detect, `UAshlineGraphicsSettings` applies **`Ashline_SteamDeck`**:

- 1280×800 fullscreen, VSync **on**, `t.MaxFPS=60`
- FSR3 Performance (`QualityMode=3`) if the AMD plugin is present, else TSR @ 77%
- Hardware RT **off** (Deck iGPU + Proton RT is a battery / hitch risk)
- Streaming pool **1800**, anisotropy 8, cheap Lumen gather
- Frame generation **off**

Manual: console `AshDeck` (Development) or the same exec in a Test build. `AshFSR` / `AshTSR` stay live in Shipping.
...
| X / Square | Reload |
| Y / Triangle | Swap |
| L1 / LB | Interact (feel-layer prompt) |
| L3 / LS click | Sprint |
| View / Share | Camera FPS/TPS |
| D-pad up | Camera |
| D-pad down | Interact |
...
| --- | --- | --- | --- |
| Preset | `AshDeck` | Deck + 40 fps | 15W is a stretch after Megascans land |
| Resolution | 1280×800 | 1152×720 | TDP, not native panel 800p pride |
| Upscaler | FSR Perf / TSR 77 | FSR Perf / TSR 50 | Never native 100% after kits |
| RT | Off | Off | |
| Frame gen | Off | Off | Adds latency on a handheld |
| VSync | On | On | |

When PR #8 merges: `[Ashline_SteamDeck DeviceProfile]` inherits **Windows** (Proton).  
