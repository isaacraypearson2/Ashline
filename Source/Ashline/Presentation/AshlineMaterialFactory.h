 ...
 *
 * Resolve order matches UAshlineContentManifest::MasterMaterialCandidates:
 * MI_Ashline{Slug} → M_Ashline{Slug} → M_{Slug} → Engine DefaultMaterial + Engine textures.
 * Missing Quixel/Fab never breaks PIE or Shipping. Loads go through AshlineLoad (quiet).
 */
UCLASS()
...
public:
	...
	static TArray<FString> MasterGraphCandidates(EAshlineMasterMaterial Master);
